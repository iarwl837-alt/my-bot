...
# استيراد الدوال من ملفك الأصلي api.py
from api import process_card, parse_cc_string, extract_clean_response

API_TOKEN = '5799226531:AAGY9ve702AFllrVQetQnIKjM2olGJIjDU8'  # ضع توكن بوتك هنا
bot = Bot(token=API_TOKEN)
storage = MemoryStorage()
dp = Dispatcher(bot, storage=storage)

# حالات FSM لتتبع خطوات المدخلات لكل خيار
...
# --- تعريف الأزرار والـ 13 خياراً ---

def get_main_keyboard():
    # القائمة الرئيسية (3 خيارات)
...
def get_checker_keyboard():
    # قائمة التشيكر الفرعية (3 خيارات)
...
def get_site_keyboard():
    # قائمة المواقع الفرعية (3 خيارات)
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(
...
async def cmd_start(message: types.Message):
    await message.answer(
        "⚡ أهلاً بك يا إنستا في لوحة التحكم الخاصة بالبوت.\nاختر أحد الأقسام الرئيسية:",
...
async def run_mass_checker(message: types.Message, state: FSMContext):
    ...
    # اربط هنا دالة الفحص الجماعي الخاصة بك
    await state.finish()
    await message.answer("اختر من القائمة الرئيسية:", reply_markup=get_main_keyboard())
...
async def run_site_single(message: types.Message, state: FSMContext):
    ...
    # اربط هنا دالة فحص الموقع الفردي
    await state.finish()
    await message.answer("اختر من القائمة الرئيسية:", reply_markup=get_main_keyboard())
...
async def run_site_mass(message: types.Message, state: FSMContext):
    ...
    # اربط هنا دالة فحص المواقع الجماعي
    await state.finish()
    await message.answer("اختر من القائمة الرئيسية:", reply_markup=get_main_keyboard())
