...
API_TOKEN = '5046453512:AAFyI-lVzFfHHdCpa6GS7fg1bVgETKk102s'  
...
class BotStates(StatesGroup):
    ...
    waiting_mass_site_file = State()  # استقبال ملف الروابط للفحص الجماعي
...
async def get_proxy_doc(message: types.Message, state: FSMContext):
    ...
    await doc.download(destination_file=proxy_path)
...
async def get_cc_doc(message: types.Message, state: FSMContext):
    ...
    await doc.download(destination_file=cc_path)
...
@dp.message_handler(state=BotStates.waiting_mass_site_file, content_types=types.ContentTypes.DOCUMENT)
async def run_mass_checker_with_sites_file(message: types.Message, state: FSMContext):
    status_msg = await message.answer("🚀 **جاري استقبال ومعالجة ملف الروابط...**")
    try:
        doc = message.document
        site_path = f"sites_{message.from_user.id}.txt"
        await doc.download(destination_file=site_path)
        ...
        if not cards or not sites:
            await status_msg.edit_text(f"❌ خطأ: ملف البطاقات ({len(cards)}) أو ملف المواقع ({len(sites)}) فارغ أو غير صالح.")
            await state.finish()
            return
            
        await status_msg.edit_text(f"🔄 **تم تحميل الملفات بنجاح!**\n- عدد البطاقات: {len(cards)}\n- عدد المواقع: {len(sites)}\nجارٍ بدء الفحص...")
        ...
        for line in cards[:5]: # فحص أول 5 بطاقات كاختبار أمان لعدم تعليق البوت
            processed += 1
        ...
        await message.answer(f"📊 **ملخص الفحص الجماعي:**\n- المواقع المفحوصة: {len(sites)}\n- المعالج: {processed}\n- مقبولة: {valid_count}\n- مرفوضة: {dead_count}")
    except Exception as e:
        await message.answer(f"❌ حدث خطأ غير متوقع أثناء المعالجة: {str(e)}")
    finally:
        await state.finish()
        await message.answer("اختر من القائمة الرئيسية:", reply_markup=get_main_keyboard())
