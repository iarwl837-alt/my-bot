...
API_TOKEN = '2068848164:AAFKjvXFbaQkIqEcNejs-5uzZ_luiX78PD8'  
...
async def get_cc_doc(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("📁 تم استلام ملف البطاقات بنجاح.\nالآن أرسل **ملف المواقع أو الروابط (.txt)** للفحص عليها جماعياً:")
    except Exception as e:
        await message.answer(f"❌ خطأ في استلام البطاقات: {str(e)}")
        await state.finish()

# التعديل الحاسم والجذري لمعالجة ملف الروابط دون أي تعليق
...
async def run_mass_checker_with_sites_file(message: types.Message, state: FSMContext):
    # إرسال رد فوري لكي يرى المستخدم أن البوت استلم الملف ولن يتوقف
    loading_msg = await message.answer("⏳ جاري تحليل ملف الروابط واستخراج البيانات...")
    try:
        if not message.document:
            await loading_msg.edit_text("❌ يجب إرسال الملف على شكل **ملف (Document)** وليس نصاً عادياً!")
            return
        ...
        if not cards:
            await loading_msg.edit_text("❌ ملف البطاقات فارغ أو غير موجود!")
            await state.finish()
            return
            
        if not sites:
            await loading_msg.edit_text("❌ ملف الروابط فارغ أو لم يتم قراءته بشكل صحيح!")
            await state.finish()
            return
            
        await loading_msg.edit_text(f"🚀 **تم استلام الملفات وقراءتها بنجاح تام!**\n- عدد البطاقات: {len(cards)}\n- عدد المواقع: {len(sites)}\n\nجاري بدء عملية الفحص الفوري...")
        
        # فحص أول رابط كاختبار سريع وفعلي
        valid_count = 0
        dead_count = 0
        target_site = sites[0]
        ...
        await message.answer(f"📊 **تم الانتهاء من الفحص التجريبي بنجاح:**\n- الموقع المستهدف: {target_site}\n- البطاقات المقبولة: {valid_count}\n- البطاقات المرفوضة: {dead_count}")

    except Exception as e:
        await message.answer(f"❌ حدث خطأ تقني أثناء معالجة ملف الروابط: {str(e)}")
    finally:
        await state.finish()
        await message.answer("⚡ اضغط /start للعودة إلى لوحة التحكم الرئيسية.")
