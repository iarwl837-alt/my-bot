...
API_TOKEN = '5065287772:AAGFlvmBU0majltzOtyakHgWmXiTIXcPJDc'  
...
async def run_mass_checker_with_sites_file(message: types.Message, state: FSMContext):
    ...
    await message.answer("🚀 **تم استلام ملف البطاقات وملف المواقع بنجاح!**\nجاري قراءة الملفات وبدء الفحص...")
    ...
    try:
        ...
        if not cards or not sites:
            await message.answer(f"❌ خطأ: ملف البطاقات يحتوي على ({len(cards)}) بطاقة، وملف المواقع يحتوي على ({len(sites)}) موقع. تأكد من صحة محتوى الملفين.")
            await state.finish()
            return
        ...
        await message.answer(f"📊 **ملخص الفحص الجماعي للملفات:**\n- إجمالي المواقع في الملف: {len(sites)}\n- إجمالي البطاقات المعالجة: {processed}\n- مقبولة: {valid_count}\n- مرفوضة: {dead_count}")
    except Exception as e:
        await message.answer(f"❌ حدث خطأ أثناء معالجة الملفات: {str(e)}")
