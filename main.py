...
API_TOKEN = '5152569183:AAG-JKh6B_wiEgfEMjkBxQvnLXzMVCEK1b8'  
...
async def run_mass_checker_with_sites_file(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if not cards or not sites:
            await message.answer(f"❌ خطأ: ملف البطاقات يحتوي على ({len(cards)}) بطاقة، وملف المواقع يحتوي على ({len(sites)}) موقع. يجدر أن لا يكون أي منهما فارغاً.")
            await state.finish()
            return
