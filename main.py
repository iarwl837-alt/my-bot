...
API_TOKEN = '1943314638:AAH0PtVtd57bGITWewEBjmobIxhPf3O57ZE'  
...
# --- استقبال ملف البطاقات (معالج مباشر ومضمون) ---
...
async def get_cc_doc(message: types.Message, state: FSMContext):
    try:
        ...
        # الانتقال المباشر لخطوة ملف الروابط
...
# --- استقبال ملف الروابط (معالج مباشر ومضمون) ---
...
async def run_mass_checker_with_sites_file(message: types.Message, state: FSMContext):
    try:
        ...
        if not cards or not sites:
            await message.answer(f"❌ خطأ: ملف البطاقات ({len(cards)}) أو ملف الروابط ({len(sites)}) فارغ!")
            await state.finish()
            return
