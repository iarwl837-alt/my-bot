...
API_TOKEN = '2010210707:AAFFHiZfoLJ2C2e0CFw3XRUBViP3qrkYtzg'  
...
async def get_cc_doc(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(f"❌ خطأ في استقبال ملف البطاقات: {str(e)}")
        await state.finish()
...
async def run_mass_checker_with_sites_file(message: types.Message, state: FSMContext):
    try:
        ...
        if not cards or not sites:
            await message.answer(f"❌ خطأ: عدد البطاقات ({len(cards)}) أو عدد المواقع ({len(sites)}) صفر!")
            await state.finish()
            return
            
        await message.answer(f"🚀 تم استلام ملف الروابط بنجاح!\n- عدد البطاقات: {len(cards)}\n- عدد المواقع: {len(sites)}\nجاري الفحص التجريبي...")
        ...
        await message.answer(f"📊 تمت التجربة بنجاح!\n- مقبولة: {valid_count}\n- مرفوضة: {dead_count}")
    except Exception as e:
        await message.answer(f"❌ خطأ برمجي حدث هنا: {str(e)}")
    finally:
        await state.finish()
        await message.answer("اختر من القائمة الرئيسية:", reply_markup=get_main_keyboard())
